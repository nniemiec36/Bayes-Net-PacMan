...
def constructBayesNet(gameState):
    ...
    var = {X_POS_VAR:X_POS_VALS, Y_POS_VAR:Y_POS_VALS, FOOD_HOUSE_VAR:HOUSE_VALS, GHOST_HOUSE_VAR:HOUSE_VALS}
    variableDomainsDict = var

    # x positions determine which house goes on which side of the board
    edges.append((X_POS_VAR, GHOST_HOUSE_VAR))
    edges.append((X_POS_VAR, FOOD_HOUSE_VAR))
    # y positions determine how the houses are vertically oriented
    edges.append((Y_POS_VAR, FOOD_HOUSE_VAR))
    edges.append((Y_POS_VAR, GHOST_HOUSE_VAR))

    # parse through all legal house positions
    for h_pos in gameState.getPossibleHouses():
        # get all wall positions
        for w_pos in gameState.getHouseWalls(h_pos):
            obs_var = OBS_VAR_TEMPLATE % w_pos
            # append to var array
            obsVars.append(obs_var)
            # set dict value of current obs value to constant
            variableDomainsDict[obs_var] = OBS_VALS
    
    # parse through all variables in observed list
    for var in obsVars:
        # populate each food and ghost house edge
...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    # each house can only be in one of four positions
    # setting the top probabilities
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)
    # setting the bottom probabilities
    yFactor.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL}, PROB_BOTH_BOTTOM)
    # setting the top left probabilities
    yFactor.setProbability({Y_POS_VAR: LEFT_TOP_VAL}, PROB_ONLY_LEFT_TOP)
    # setting the bottom left probabilities
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    best_prob = 0
    best_assignment = factor.getAllPossibleAssignmentDicts()[0]#Either first is best assignment or some other assignment is best
    for assignment in factor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment)
        if prob > best_prob: #checks if current assignment has the best probibility of those checked
