...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # get list of all conditional probability tables
        factor_list = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # parse through the elimination order
        for e in eliminationOrder:
            # initialize the factors
            factor_list, new_factor = joinFactorsByVariable(factor_list, e)
            # get rid of those with length greater than 1
            length = len(new_factor.unconditionedVariables())
            if length > 1:
                var = eliminate(new_factor, e)
                # create new list with eliminated vars
                factor_list.append(var)

        # append all factors
        joined = joinFactors(factor_list)

        # return the normalized factors
